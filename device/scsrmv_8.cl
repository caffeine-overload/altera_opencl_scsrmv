...
#pragma unroll
        ...
        float accumulator[block_height];
#pragma unroll
        for(int x = 0; x < block_height; x++){
            accumulator[x] = y[block_row_ptr + x] * beta;
        }
        float shifter[block_height][FADD_LATENCY] = {{ZERO}};
...
#pragma unroll
            for(int x = 0; x < block_height; x++){
                float new_value = shifter[x][0] + sum[x];
#pragma unroll
                for(int k = 0; k < FADD_LATENCY - 1; k++){
                    shifter[x][k] = shifter[x][k + 1];
                }
                shifter[x][FADD_LATENCY - 1] = new_value;
            }
        }
#pragma unroll
        for(int x = 0; x < block_height; x++){
#pragma unroll
            for(int k = 0; k < FADD_LATENCY; k++){
                accumulator[x] += shifter[x][k];
            }

            y[block_row_ptr + x] = accumulator[x];
        }
    }
